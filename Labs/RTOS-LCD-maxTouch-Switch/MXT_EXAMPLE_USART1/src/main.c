...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
  ...
  {
    t_but but = botoes[i];      
    if ((x >=  but.x && x <= but.x + but.width))
		if (y >= but.y && y <= but.y + but.height)
	      return i;
  }
  return -1;
...
void task_lcd(void){
	...
	t_but but0 = {120, 75, COLOR_TOMATO, COLOR_BLACK, ILI9488_LCD_WIDTH/2, 40, 1 };
	t_but but1 = {120, 75, COLOR_BLUE, COLOR_BLACK, ILI9488_LCD_WIDTH/2, 140, 1 };
	t_but but2 = {120, 75, COLOR_GREEN, COLOR_BLACK, ILI9488_LCD_WIDTH/2, 240, 1 };
